...
export async function generateMetadata({ params: { slug } }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://searchgrowai.com/blog/${slug}`,
    },
    twitter: {
      title,
      description,
  ...
  return (
    <section className="article max-w-2xl lg:my-8 mx-auto px-4 md:px-8 dark:text-white">
      ...
      <div className="flex flex-wrap gap-2 items-center w-full text-sm dark:text-slate-400">
        <span>
          Published{' '}
          {new Date(post.createdAt || post.createdAt).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}
        </span>
      ...
      <div className="relative flex justify-center items-center w-full aspect-video mt-2 text-center rounded-xl overflow-hidden">
        <Image src={post.featuredImage} alt={post.title} layout="fill" objectFit="cover" className='!inset-auto' />
      </div>
