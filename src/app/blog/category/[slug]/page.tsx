...
export async function generateMetadata({ params: { slug } }: { params: { slug: string } }): Promise<Metadata> {
  const title = `${deslugify(slug)} - Searchgrowai Blog`;
  return {
    title,
    ...
    openGraph: {
      ...
      // description: '',
      // images: [],
      url: `https://searchgrowai.com/blog/category/${slug}`,
    },
    twitter: {
      title,
      // description: '',
...
export default async function Category({
  ...
  return (
    <section className="max-w-3xl my-8 lg:mt-10 mx-auto px-4 md:px-8 dark:text-white">
      <div className="flex flex-wrap items-center gap-2 w-full dark:text-slate-400 text-sm mb-4">
